...
run_resamp_inference_phase23 <- function(mu0, mu1, mu2, sigma, n1, n2, alternative,
                                         ...
                                         alpha_LCL = 0.025,
                                         n_iter_dose1_done, n_iter_dose2_done,
                                         htest_method = "mean") {
  ...
  LCL_delta_hat_naive = delta_hat_naive - qt(1 - alpha_LCL, df = n - 2) * sqrt(var_delta_hat_naive)
  ...
  while (b1 <= B1) {
    ...
    if (d_boot != d){
      ...
      #bias_vals <- c(bias_vals, delta_hat_d_star_s1 - delta_hat_naive_s1)
      b1 = b1 + 1
